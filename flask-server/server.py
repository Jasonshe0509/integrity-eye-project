...
def compare_months():
    ...
    # Filter by selected year and month
    current_month_count = df[(df['TARIKH PAPARAN KEPUTUSAN'].dt.month == month) & (df['TARIKH PAPARAN KEPUTUSAN'].dt.year == year)].shape[0]

    # Get previous month
    previous_month = month - 1 if month > 1 else 12
    previous_year = year if month > 1 else year - 1

    previous_month_count = df[(df['TARIKH PAPARAN KEPUTUSAN'].dt.month == previous_month) & (df['TARIKH PAPARAN KEPUTUSAN'].dt.year == previous_year)].shape[0]
